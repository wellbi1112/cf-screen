...
## LIVE 현재 대회 현황
대회가 `진행중`이면 최다참여 · 등급별 1위(회원별 베스트 스코어) · 롱기스트(남) · 롱기스트(여) · 니어핀을 보여줍니다. 롱기스트·니어핀은 기록부문 탭에 부문 이름을 `롱기스트(남)`, `롱기스트(여)`, `니어핀`으로 적고 닉네임·기록을 넣으면 됩니다. 라운드 탭에 `보정타` 열을 추가해 값을 넣으면 보정타 기준으로 정렬되고, 없으면 실타 기준입니다.
